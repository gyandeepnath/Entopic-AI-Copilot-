...
**Covers build:** Entopic v1.5.0, KB v1.3.1 — 127 loaded files (104 in `js/`, 23 in
`knowledge/`), 394 conditions across 9 domains, 103 test files.
...
## 9f. The adversarial harnesses (`tools/stress/`)

Phase 11. `attack.js` was one file covering storage, migrations, the engine and
the red flags. Eight areas had **no adversarial coverage at all**, and the
pattern in the misses was consistent: each was one the unit suite could not
reach without a browser, a hostile input, or a second device.

| Harness | Attacks | Asks |
|---|---|---|
...
**The sandbox is the load-bearing part.** `lib.js` deliberately does NOT inject
`Object`/`JSON`/`Array` from the host realm. A vm context has its own standard
built-ins, and injecting the host's breaks realm identity: an object literal
created inside the sandbox inherits the SANDBOX realm's `Object.prototype`,
while the injected `Object.prototype` belongs to the host. Polluting the latter
leaves `{}` untouched, so every prototype-pollution attack passes while proving
nothing. Measured:
...
Fixing this exposed four real defects in one run. `tests/prototype-pollution.test.js`
pins the root cause so re-injecting them cannot silently disarm the class again.

**Rules every harness follows** (inherited from attack.js):
1. Attack the REAL modules; mock only the browser around them.
2. Assert what MUST hold, never what currently does.
3. Crashing on garbage is acceptable. Silent wrong answers are not.
4. No invented clinical values.
5. **Positive controls are mandatory** where a fix could take the form of
   refusing everything. `privacy.js` group T exists because the first run
   reported all-clean while the code under test was withholding every value —
   the leak assertions passed because there was nothing left to leak.

**Findings are promoted into `tests/`** once fixed, because the harnesses run
manually and `node --test` is the CI gate. Phase 11 promoted six files:
research de-identification, auth portability, prototype pollution, escaping on
the clinical surfaces, sync merge integrity, and OSDI scoring.
...
**Root** — `index.html` (698), `css/entopic.css` (2604)

**`/knowledge`** — 23 files, loaded first, in this order:

- `clinical-thresholds.js` (523)
- `red-flags.js` (265)
- `surface.js` (351)
- `corneal.js` (293)
- `retina.js` (266)
...
- `storage-mirror.js` (333)
- `local-vault.js` (853)
- `vault-admin-recovery.js` (258)
- `cloud-config.js` (68)
- `kb-remote.js` (400)
- `kb-signoffs.js` (287)
- `kb-review.js` (241)
